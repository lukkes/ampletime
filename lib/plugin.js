...
import {appendToHeading} from "./amplefocus/logWriter.js";
  ...
  async onEmbedCall(app, ...args) {
    if (args.length === 1 && args[0] === "end-cycle") {
      ...
      if (args[0] === "clipboard") {
        ...
        fetch(base64Image)
            .then(res => res.blob())
            .then(blob => _dataURLFromBlob(blob))
            .then(dataURL => app.attachNoteMedia(noteHandle, dataURL))
            .then(fileURL => appendToHeading(app, "Session debrief", `![](${fileURL})`))
