...
import { _isTaskRunning, _logStartTime, _getTaskDurations, _stopTask } from "./tasks.js"
import { _getCurrentTime, _getFormattedDate } from "./date-time.js";
import {_createLegendSquare, _generatePie} from "./reports.js";
...
const plugin = {
  ...
  insertText: {
    "Start This Task": {
      async run(app) {
        try { await this._startTask(app); } catch (err) { console.log(err); await app.alert(err); }
      },
  ...
  async _start(app, target) {
    ...
    let runningTaskDuration = await _getTaskDurations(
      app, dash, _makeNoteLink(target), startDate, endDate
    );
  ...
  async _generateReport(app, reportType) {
    ...
    for (let i = 0; i < taskDurations.length; i++) {
      let fileURL = await app.attachNoteMedia(
        resultsHandle, await _createLegendSquare(this.options.colors[i], this.options)
      );
