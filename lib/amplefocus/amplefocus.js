import {
    ...
    _isTaskRunning,
    _logStartTime, _readDasbhoard,
    _stopTask, writeDashboard
...
export async function _promptEnergyMorale(app, message) {
    let [energy, morale] = await app.prompt(message,
        {
            inputs: [
                {
...
export async function _startSession(app, options, dash, startTime, cycles, firstCycle) {
...
export async function _handleBreakPhase(app, options, dash, focusNote, workEndTime, breakEndTime, cycleIndex, cycles) {
