...
export function initAmplefocus(app, options) {
    ...
    for (let pair of Object.entries(options.settings)) {
        ...
        if (app.settings[setting]) {
            options[option] = app.settings[setting] * 60 * 1000;
        }
    }
    markStopped();
}
...
export function _calculateEndTime(options, startTime, cycles) {
    ...
    const totalMinutes = Math.floor(totalTime / 60000) % 60;
...
export async function _handleBreakPhase(app, options, dash, breakEndTime, cycleIndex, cycles, handlePastCylces=false, prompt=true) {
