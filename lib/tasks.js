...
export async function _getTaskDistribution(app, dash, target, startDate, endDate) {
    ...
    for (let entry of entries) {
        ...
        if (task.urgent && task.important) taskDistribution.q1.push(task);
        else if (!task.urgent && task.important) taskDistribution.q2.push(task);
        else if (task.urgent && !task.important) taskDistribution.q3.push(task);
        else if (!task.urgent && !task.important) taskDistribution.q4.push(task);
    }
