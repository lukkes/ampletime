...
// ==== MAIN ENTRY POINTS ====
//===================================================================================
import {_getTaskDistribution, _getTaskDurations, _isTaskRunning, _logStartTime, _stopTask} from "./tasks.js";
import {_getEntryName} from "./entries.js";
import {_createTableHeader, _makeNoteLink} from "./markdown.js";
...
export async function _start(app, options, target) {
    ...
    await _logStartTime(app, dash, toStart, currentTime, options);
...
export async function _ensureDashboardNote(app, options) {
    ...
    const timeEntriesSection = sections.find(
        (section) => section.heading &&
            section.heading.text === options.sectionTitleDashboardTimeEntries
    );
    ...
    if (!timeEntriesSection) {
        await app.insertNoteContent(
            dash,
            `\n## ${options.sectionTitleDashboardTimeEntries}\n`,
            { atEnd: true }
        );
